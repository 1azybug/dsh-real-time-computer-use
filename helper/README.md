...
- **帧间隔上界（2026-09-25 定）**：部署目标是"任何一帧的采集间隔 ≤ **33.333 ms**"。当前 `interval_ms=16`
  （62.5 fps）留 17.3 ms 余量，原先必然越界的那类抖动（片内第 100 帧预建编码器与采集并发，10–15 ms）不再越界。
  实测（2560×1440 / h264 / dxgi、全屏动画负载 + 第二个 helper 实例陪跑）：**16 ms 档 1/11217 帧**违约（0.009%，
  那 1 帧是 64.7 ms 的**系统级尖峰**，与预建无关）；对照 25 ms 档 4/2408（0.17%，**全部**是预建那一帧）。
  **不保证零违约**——系统级停顿在任何档位都可能越界。代价：单核 CPU 52.2% → 79.1%、录像体积 ×1.5。
- **封片已移出采集线程**：`Finish`+`Dispose`（本机负载下实测 **176 ms**）由专职封片线程承担，采集线程只入队；
  同条件对照（各 ~2400 帧），**片边界（每 150 帧一次）的违约从 9 帧降到 0 帧**，总违约 14 → 4。
  可行性先验：主线程建+写 / 后台线程 Finish+Dispose，异步产出的片字节与同步**逐字节相同**，
  且主线程 25 ms 节拍的最坏间隔只比无封片基线多 **4.1 ms**。
- **残余抖动没有免费解法**：试过"常驻预建线程 + 采集 `AboveNormal` / 后台 `BelowNormal` + 删掉第 100 帧触发点"
  的实验版，结果只是把同一处抖动挪到**片内第 118 帧**，并新增片首停顿（最差 55.9 ms，本版 37.6 ms）⇒ **未采纳**。
  根因是"**每片重建编码器**"这件事本身与采集并发；要压到零违约，需要不再每片重建
  （sink writer 连续写、把"片"做成索引 + 内存块的逻辑分段），属**未实现**方向。
