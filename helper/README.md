...
- **帧间隔上界未完全达标（2026-09-25 实测）**：部署目标是"任何一帧的采集间隔 ≤ **33.333 ms**"
  （`interval_ms=25`，40 fps 目标）。条件：2560×1440 / h264 / dxgi、全屏动画负载 + 第二个 helper 实例陪跑，
  每 ~2400 帧约 **4–5 帧**违约（35–37 ms，达标率 **99.8%**）。违约帧**全部落在片内第 100 帧**——
  即后台**预建下一个编码器**（`MemoryEncoder` 创建约 220 ms）与采集线程并发的那一帧。**不是**采集或编码吞吐不够。
