/**
 * dsh-computer-use —— Computer Use（Windows）：让 Agent 看见 Windows 桌面并像人一样操作它。
 *
...
const DEFAULTS = {
   ...
   * 它是"只返回一张图"的同族绕道——能把任意一张图裁成一块、1:1 交给模型，从而绕开 `screen_grid`
   * 的分块形态。拦截**只针对 region 这一条路**：不带 region 的 read_image（整图读）照常可用。
   * 默认 false 是给通用部署留的——别的部署没有这条口径，不该替它决定。
   */
  denyReadImage: true,
  /**
...
export function apply(ctx, config = {}) {
  ...
  // `read_image` 的 `region` 是同族的"单图"绕道：把任意一张图裁成一块、1:1 交给模型，
  // 于是 `screen_grid` 的分块形态可以被绕开。用 guard 按「工具名 + 参数」拒绝——**只堵 region 这一条**，
  // 不带 region 的 read_image（整图读）照常可用；拒绝文案里给出正确替代路径，否则模型会以为工具坏了。
